<template>
	<v-container fluid py-0 px-0>
		<template v-if="!loadingChatModule">
			...
			<v-layout row wrap class="chat-layout px-3">
				<v-flex xl3 lg3 md4 sm0 xs0 class="chat-sidebar">
					<chat-sidebar></chat-sidebar>
				</v-flex>
				<v-flex xl9 lg9 md8 sm12 xs12 class="chat-main">
					<chat-area></chat-area>
				</v-flex>
			</v-layout>
		</template>
...
export default {
  ...
  computed: {
    ...mapGetters(["loadingChatModule", "selectedChat", "token"]),
    chatSidebar: {
      get() {
